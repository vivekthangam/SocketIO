...
class SocketIO implements MethodCallHandler {
    ...
    private SocketIO(MethodChannel channel, Options options) {
        this.channel = channel;
        this.options = options;
        log("Connecting to... "+options.uri);
        socket = SocketIO.manager.socket(options.namespace);
    }

    static SocketIO getInstance(Registrar registrar, Options options) throws URISyntaxException{
        ...
        SocketIO.manager = new Manager(new URI(options.uri), options);
        SocketIO _socket = new SocketIO(channel, options);
