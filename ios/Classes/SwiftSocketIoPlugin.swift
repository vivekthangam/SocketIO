...
public class SwiftSocketIoPlugin: NSObject, FlutterPlugin {
    ...
    var instances: [Int: Socket];
    var currentIndex: Int;
    let registrar: FlutterPluginRegistrar;

    init(_ _registrar: FlutterPluginRegistrar){
        registrar = _registrar
        instances = [Int: Socket]()
        currentIndex = 0;
    }

    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "_socket_io", binaryMessenger: registrar.messenger())
        let instance = SwiftSocketIoPlugin(registrar)
        ...
        switch (call.method) {
            case "newInstance":
                ...
                let config:SocketIOClientConfig
                    = SocketIOClientConfig(newIndex, uri: arguments["uri"] as! String,
                ...
                instances[newIndex] = Socket.getInstance(registrar, config)
                currentIndex += 1
                result(newIndex)
            case "clearInstance":
                if(arguments["id"] == nil){
