...
public class SocketIO: NSObject, FlutterPlugin {
    
    let socket: SocketIOClient
    let channel: FlutterMethodChannel
    let manager: SocketManager
    let config: SocketIOClientConfig
    ...
    public init(_ channel:FlutterMethodChannel, _ config:SocketIOClientConfig) {
    ...
    public static func getInstance(_ registrar: FlutterPluginRegistrar, _ config:SocketIOClientConfig) ->  SocketIO{
        let channel = FlutterMethodChannel(name: "adhara_socket_io:socket:"+String(config.adharaId), binaryMessenger: registrar.messenger())
        let instance = SocketIO(channel, config)
...
public class SocketIOClientConfig: NSObject{
